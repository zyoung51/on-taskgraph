...
function taskRunnerFactory(
            ...
            self.rpcObj.schedulerChannel.waitForIdentified({ host: hosts[0] }, function(err) {
                ...
                tchannelJSON.send(self.rpcObj.schedulerChannel.request({
                    headers: {
                        ...
                        uuid: self.taskRunnerId
                    },
                    serviceName: 'scheduler',
                    hasNoParent: true,
                    timeout: 5000,
                    host: hosts[0]
                }), 'finishedTask', { domain: self.domain, uuid: self.taskRunnerId, pending: Object.keys(self.activeTasks).length }, data, function(err, resp) {
                    if(!err && resp.ok) {
                        //console.log( { domain: self.domain, uuid: self.taskRunnerId, pending: Object.keys(self.activeTasks).length });
                        completed += 1;
                        console.log('status - rcv: ' + rcv + ', completed: ' + completed);
                        return resolve(resp);
                    }
                    console.log('error finishing task', err);
                    reject(err);
                });
            });
        })
    ...
    TaskRunner.prototype.rpcScheduleTask = function(context, req, head, body, callback) {
        var found = _.find(context.rpcObj.schedulers, {uuid: head.uuid});
        if(found) {
