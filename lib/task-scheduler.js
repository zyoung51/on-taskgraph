...
function taskSchedulerFactory(
    ...
    function TaskScheduler(options) {
        ...
        this.timerReadyStream = new Rx.Subject();
    ...
    TaskScheduler.prototype.initializePipeline = function() {
        ...
        // Outputs to this.timerReadyStream
        this.createTimerPollerSubscription(this.timerReadyStream)
        .subscribe(
            this.handleStreamDebug.bind(this, 'Graph finished'),
            this.handleStreamError.bind(this, 'Error at check graph finished stream')
        );
    ...
    TaskScheduler.prototype.publishGraphFinished = function(graph) {
        ...
        .then(function() {
            var timerId = _.get(graph, 'context.timerId');
            if(timerId) {
                return self.completeTimerGraph(timerId, graph);
            }
        })
    ...
    var timerMap = {};
    TaskScheduler.prototype.createTimerPollerSubscription = function(timerReadyStream) {
        var self = this;
        
        return Rx.Observable.interval( 1000 )
        .takeWhile(self.isRunning.bind(self))
        .map(store.findReadyTimers.bind(store, self.concurrencyMaximums.timerGraphs.max))
        .mergeLossy(self.concurrencyMaximums.timerGraphs)
        .filter(function(tasks) { return !_.isEmpty(tasks); })
        .flatMap(self.handleTimers.bind(self))
        .catch(this.handleStreamError.bind(this, 'Error processing timers'));
    };

    TaskScheduler.prototype.handleTimers = function(timers) {
        var self = this;
        var doTimers = _.filter(timers, function(timer) {
            if(!timer.node) {
                return true;
            }
            if( !_.has(timerMap, timer.node) ) {
                timerMap[timer.node] = {
                    inflight: [],
                    stats: {},
                    starttime: {},
            ...
            if( _.includes(timerMap[timer.node].inflight, timer.config.command )) {
                return false;
            }
            timerMap[timer.node].inflight.push(timer.config.command);
            return true;
        });

        console.log(util.inspect(timerMap, false, null));

        return Rx.Observable.just(doTimers)
        .flatMap(Promise.map.bind(Promise, doTimers, self.computeUpdate.bind(self)))
        .flatMap(Promise.map.bind(Promise, doTimers, self.postTimer.bind(self)));
    };

    TaskScheduler.prototype.completeTimerGraph = function(timerId, graph) {
        return store.updateTimerStatus(timerId, graph._status, graph.createdAt, graph.updatedAt)
        .then(function(timer) {
            var obj = timerMap[timer.node];
            _.pull(obj.inflight, timer.config.command);
            _.set(obj.stats, timer.config.command, _.get(obj.stats, timer.config.command, 0) + 1);
            _.set(obj.time, timer.config.command, process.hrtime(_.get(obj.starttime, timer.config.command)));
            return store.deleteGraph(graph.instanceId);
        })
        .catch(function(error) {
            logger.error('Error updating periodic graph status', {
                graphId: graph.instanceId,
                _status: graph._status,
                error: error
            });
    ...
    TaskScheduler.prototype.computeUpdate = function(timer) {
        var nextScheduled = new Date();
        return store.updateTimerNext(timer, new Date(nextScheduled.valueOf() + timer.pollInterval));
    };

    var defCache = {};

    TaskScheduler.prototype.postTimer = function(timer) {
        var self = this;
        timerMap[timer.node].starttime[timer.config.command] = process.hrtime();
        return Promise.resolve(_.get(defCache, timer.name))
        .then(function(definition) {
          if(!definition) {
            return store.getGraphDefinitions(timer.name)
            .then(function(definition) {
              _.set(defCache, timer.name, definition);
              return definition;
            });
          }
          return definition;
        ...
        .then(function(definition) {
          if(!_.isEmpty(definition)) {
              return TaskGraph.create( self.domain, {
                  ...
                  options: {
                      'run-command': _.merge({}, { nodeId: timer.node.toString() }, timer.config)
                  },
                  context: { timerId: timer._id ? timer._id : timer.id }
              });
          }
        })
        .then(function(graph) {
    ...
    TaskScheduler.prototype.cleanupTimers = function() {
        return Rx.Observable.just();
    };
