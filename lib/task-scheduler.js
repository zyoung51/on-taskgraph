    ...
    TaskScheduler.prototype.publishScheduleTaskEvent = function(data) {
        ...
        return Promise.try(function() {
            if(_.isEmpty(self.rpcObj.runners)) {
                logger.debug('Attempting to schedule event with no available runners');
                return;
            }

            return new Promise(function(resolve, reject) {
                var peers = _.keys(self.rpcObj.runners);
                self.rpcObj.runnerChannel.waitForIdentified({ host: peers[0] }, function(err) {
                    ...
                    tchannelJSON.send(self.rpcObj.runnerChannel.request({
                        headers: {
                            cn: 'publishScheduleTaskEvent'
                        },
                        serviceName: 'runner',
                        hasNoParent: true,
                        timeout: 5000
                    }), 'scheduleTask', { domain: self.domain }, data, function(err, resp) {
                        if(!err && resp.ok) {
                            return resolve(resp);
                        }
                        reject(err);
        ...
        return new Promise(function(resolve, reject) {
            self.rpcObj.runnerChannel.waitForIdentified({ host: runner.hostport }, function(err) {
                ...
                tchannelJSON.send(self.rpcObj.runnerChannel.request({
                    ...
                    hasNoParent: true,
                    timeout: 5000
                }), 'cancelTask', { domain: self.domain }, task, function(err, resp) {
                    if(!err && resp.ok) {
                        return resolve(resp);
                    }
                    reject(err);
