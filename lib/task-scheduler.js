...
function taskSchedulerFactory(
    ...
    TaskScheduler.prototype.completeTimerGraph = function(timerId, graph) {
        ...
        .then(function(timer) {
            _.pull(timerMap[timer.node].inflight, timer.config.command);
            _.set(timerMap[timer.node].stats, timer.config.command, _.get(timerMap[timer.node].stats, timer.config.command, 0) + 1);
